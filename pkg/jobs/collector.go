...
const (
	...
	// job headers
	VulCollectorName = "vul.collector.name"
	VulAutoCreated   = "vul.automatic.created"
	VulResourceName  = "vul.resource.name"
	VulResourceKind  = "vul.resource.kind"
)
...
func (jb *jobCollector) ApplyAndCollect(ctx context.Context, nodeName string) (string, error) {
	...
	_, err = jb.getVulNamespace(ctx)
	if err != nil {
		if k8sapierror.IsNotFound(err) {
			vulNamespace := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: jb.namespace}}
			_, err = jb.cluster.GetK8sClientSet().CoreV1().Namespaces().Create(ctx, vulNamespace, metav1.CreateOptions{})
			if err != nil {
				return "", err
			}
		}
...
func (jb *jobCollector) deleteVulNamespace(ctx context.Context) {
	background := metav1.DeletePropagationBackground
...
func (jb *jobCollector) getVulNamespace(ctx context.Context) (*corev1.Namespace, error) {
